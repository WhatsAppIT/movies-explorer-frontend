import img1 from '../images/pic__COLOR_pic.svg';
import img2 from '../images/pic__COLOR_pic (1).svg';
import img3 from '../images/pic__COLOR_pic (2).svg';
import img4 from '../images/pic__COLOR_pic (3).svg';
import img5 from '../images/pic__COLOR_pic (4).svg';
import img6 from '../images/pic__COLOR_pic (5).svg';
import img7 from '../images/pic__COLOR_pic (6).svg';
import img8 from '../images/pic__COLOR_pic (7).svg';
import img9 from '../images/pic__COLOR_pic (8).svg';
import img10 from '../images/pic__COLOR_pic (9).svg';
import img11 from '../images/pic__COLOR_pic (10).svg';
import img12 from '../images/pic__COLOR_pic (11).svg';

const cards = [
  {
    movieId: 1,
    nameRU: '33 слова о дизайне',
    duration: 77,
    image: img1,
    link: 'movie-explorer/src/images/save3d.svg',
  },
  {
    movieId: 2,
    nameRU: 'Киноальманах «100 лет дизайна»',
    duration: 77,
    image: img2,
  },
  {
    movieId: 3,
    nameRU: 'В погоне за Бенкси',
    duration: 77,
    image: img3,
  },
  {
    movieId: 4,
    nameRU: 'Баския: Взрыв реальности',
    duration: 77,
    image: img4,
  },
  {
    movieId: 5,
    nameRU: 'Бег это свобода',
    duration: 77,
    image: img5,
  },
  {
    movieId: 6,
    nameRU: 'Книготорговцы',
    duration: 77,
    image: img6,
  },
  {
    movieId: 7,
    nameRU: 'Когда я думаю о Германии ночью',
    duration: 77,
    image: img7,
  },
  {
    movieId: 8,
    nameRU: 'Gimme Danger: История Игги и The Stooges',
    duration: 77,
    image: img8,
  },
  {
    movieId: 9,
    nameRU: 'Дженис: Маленькая девочка грустит',
    duration: 77,
    image: img9,
  },
  {
    movieId: 10,
    nameRU: 'Соберись перед прыжком',
    duration: 77,
    image: img10,
  },
  {
    movieId: 11,
    nameRU: 'Пи Джей Харви: A dog called money',
    duration: 77,
    image: img11,
  },
  {
    movieId: 12,
    nameRU: 'По волнам: Искусство звука в кино',
    duration: 77,
    image: img12,
  },
];

export default cards;
